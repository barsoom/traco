module Traco
  module LocaleRegularExpressions
    UNMODIFIED = /[a-zA-Z]{2}(?:-[a-zA-Z]{2})?/
    RUBY = /[a-zA-Z]{2}(?:_[a-zA-Z]{2})?/
  end
  ...
  def self.attribute_name_with_locale(attribute_name, locale)
    if Traco.config.attribute_name_format == :ruby
  ...
  def self.locale_from_column_name_part(locale)
    if Traco.config.attribute_name_format == :ruby
      locale = locale.to_s.split('_').each_with_index.map do |locale_part, index|
  ...
  module ClassMethods
    ...
    def locale_regular_expression_for_attribute_name
      if Traco.config.attribute_name_format == :ruby
        Traco::LocaleRegularExpressions::RUBY
      else
        Traco::LocaleRegularExpressions::UNMODIFIED
      end
    end
  end
end
